...
int main(int argc, char* argv[])
{
	...
	fo = fopen(fn, "w");
	for (b = 0; b < 4; ++b) {
		keptBetas[b] = (double*)calloc(NCOMPONENTS, sizeof(double));
		fitted4and5Betas[b] = (double*)calloc(NCOMPONENTS, sizeof(double));
	}
	for (b = 0; b < 4; ++b) {
		target = baseNames[b];
		l = 0;
		...
		for (comp = 0; comp < l; ++comp)
			model.toFit[comp] = model.toUse[comp] = 1;
		setStartingBetasFromCounts(&model);
		evaluateModel(fo, &model, line, useThese, 0);
	...
	sprintf(fn, "modelMutation.4and5.txt");
	fo = fopen(fn, "w");
	fclose(fo);
