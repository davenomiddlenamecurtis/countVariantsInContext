...
class lrfModelMaximiser
{
public:
	double operator() (const column_vector& arg) const;
	double (*func)(void);
	lrfModelMaximiser(double (*f)(void)) { func = f; }
};
...
double lrfModelMaximiser::operator() (const column_vector& c) const
{
	int i;
	double f;
	for (i = 0; i<c.nr(); ++i)
...
Need to include F in these calculations.
...
double lrfRidgePenaltyModel::penaltyFunction()
{
	double penalty;
	int c;
	penalty = 0;
...
//	d = find_min_using_approximate_derivatives(cg_search_strategy(),
	d = find_min_using_approximate_derivatives(lbfgs_search_strategy(10),
			...
			lrfModelMaximiser(func), starting_point, -20, derivative_eps);
...
double lrfModel::getLnL()
{
	...
	for (r = 0; r < nRow; ++r)
	{
		t[r] = 0; // t is log(odds). Y is fraction of successes, F is number of trials
		for (c = 0; c < nCol; ++c)
			if (toUse[c])
				t[r] += beta[c] * X[r][c];
		if (toUse[nCol])
			t[r] += beta[nCol]; // I think this is the intercept
		if (t[r] > tLimit) // approximate log likelihood with -t if it is very extreme
		{
			if (Y[r] == 1)
				ll = 0;
			else
...
int lrfModel::init(int r, int c)
{
	...
	if (X == 0)
	{
		dcerror(1, "Memory allocation error in lrfModel::init(), r=%d c=%d", r, c);
		return 0;
	}
	for (rr = 0; rr < r; ++rr)
	{
